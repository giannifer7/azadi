...
use super::super::{run_single_pass, ProcessError, SinglePassArgs};
use std::fs;
...
#[test]
fn run_single_pass_force_generated_bypasses_incremental_skip() {
    let tmp = tempdir().unwrap();
    let src = tmp.path().join("input.adoc");
    let gen_dir = tmp.path().join("gen_out");
    fs::create_dir(&gen_dir).unwrap();
    ...
    let make_args = |force_generated| SinglePassArgs {
        inputs: vec![src.file_name().unwrap().into()],
        directory: None,
        input_dir: tmp.path().to_path_buf(),
        gen_dir: gen_dir.clone(),
    ...
    run_single_pass(make_args(false)).unwrap();
    fs::write(&out_file, "locally modified but source unchanged\n").unwrap();

    run_single_pass(make_args(true)).unwrap();
