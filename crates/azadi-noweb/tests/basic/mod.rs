// crates/azadi-noweb/tests/basic.rs
use super::common::{
    TestSetup, BASIC_CHUNK, EMPTY_CHUNK, INDENTED_CHUNK, MULTI_COMMENT_CHUNKS, NESTED_CHUNKS,
    PYTHON_CODE, SEQUENTIAL_CHUNKS, TWO_CHUNKS,
};
...
#[test]
fn test_nested_chunk_expansion() -> Result<(), AzadiError> {
    let mut setup = TestSetup::new(&["#"]);
...
#[test]
fn test_indentation_preservation() -> Result<(), AzadiError> {
    let mut setup = TestSetup::new(&["#"]);
...
#[test]
fn test_complex_indentation() -> Result<(), AzadiError> {
    let mut setup = TestSetup::new(&["#"]);
...
#[test]
fn test_sequential_chunks() -> Result<(), AzadiError> {
    let mut setup = TestSetup::new(&["#"]);
