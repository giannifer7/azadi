...
enum Error {
    #[error("noweb database operation failed: {source}")]
    ...
    #[diagnostic(code(weaveback::query))]
    Api {
        #[from]
        #[source]
        source: weaveback_api::query::ApiError,
    },
    #[error("lookup operation failed")]
    #[diagnostic(code(weaveback::lookup))]
    Lookup {
        #[from]
        #[source]
        source: weaveback_api::lookup::LookupError,
    },
    #[error("lint failed: {message}")]
    #[diagnostic(code(weaveback::lint))]
    Lint { message: String },
    #[error("coverage operation failed")]
    #[diagnostic(code(weaveback::coverage))]
    Coverage {
        #[from]
        #[source]
        source: weaveback_api::coverage::CoverageApiError,
    },
}

impl From<weaveback_tangle::db::DbError> for Error {
    fn from(e: weaveback_tangle::db::DbError) -> Self {
        Error::Noweb { source: WeavebackError::Db(e) }
    }
}
// @
```
...
fn run_tag_only(
    ...
    match weaveback_tangle::db::WeavebackDb::open(&db_path) {
        ...
        Err(e) => return Err(Error::Noweb { source: WeavebackError::Db(e) }),
    }
    Ok(())
}
    ...
    match cli.command {
        ...
        Commands::Lint { paths, strict, rule, json } => {
            weaveback_api::lint::run_lint(paths, strict, rule, json)
                .map_err(|message| Error::Lint { message })?;
        }
