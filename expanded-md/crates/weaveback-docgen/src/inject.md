...
`rewrite_doc_links` rewrites every local `href="….adoc…"`, `href="….md…"`,
and `href="….wvb…"` to `href="….html…"`. Source-document links use their
source extensions by convention, but both rendered documentation trees use
`.html`, so a simple regex substitution is needed after rendering.
...
See [xref.wvb](xref.md) for how the graph is built and
[weaveback_docgen.wvb](weaveback_docgen.md) for the module map.

## document-link → .html rewriting

The regex matches local source-document hrefs and replaces the extension.  It
runs over all HTML files under `out_dir`.
