...
`render_markdown_docs` renders the Markdown projection tree produced by the
two-pass `.wvb` pipeline.  It is intentionally separate from the AsciiDoc
renderer so the two HTML sites can coexist (`docs/html` and `docs/html-md`) and
CI can verify both.

YAML front matter is stripped before pulldown-cmark sees the document; the
metadata remains useful for repository browsing, but it is not body content.

Markdown diagram handling is deliberately format-native: fenced `plantuml` and
`d2` blocks are replaced with Markdown image syntax before pulldown-cmark runs.
This keeps the Markdown output neutral while still sharing the same SVG cache
...
fn preprocess_markdown_diagrams_renders_d2_to_markdown_image() {
    ...
    let diagram = "a -> b\n";
    let svg_name = format!("d2-{}.svg", blake3::hash(diagram.as_bytes()).to_hex());
    fs::write(cache.join(&svg_name), "<svg>d2-md</svg>").expect("cached svg");
    ...
    assert_eq!(result, format!("![D2 diagram]({svg_name})\n"));
