...
#
# Installs the azadi + asciidoc grammars and queries for Neovim (nvim-treesitter).
...
#   :TSInstall azadi asciidoc asciidoc_inline
...
print("Installing azadi + asciidoc grammars for Neovim...")

# ── 1. azadi queries ──────────────────────────────────────────────────────────
azadi_q = nvim_cfg / "queries" / "azadi"
azadi_q.mkdir(parents=True, exist_ok=True)
for name in ("highlights.scm", "injections.scm"):
    content = ";; extends\n" + (grammar_dir / "queries" / name).read_text()
    (azadi_q / name).write_text(content)
print(f"Copied azadi queries to {azadi_q}")

# ── 2. azadi.lua ──────────────────────────────────────────────────────────────
plugin_dir.mkdir(parents=True, exist_ok=True)
lua = (script_dir / "azadi.lua").read_text().replace("__GRAMMAR_DIR__", str(grammar_dir))
(plugin_dir / "azadi.lua").write_text(lua)
print(f"Installed {plugin_dir / 'azadi.lua'}")
...
# ── 5. azadi injection into [source,azadi] asciidoc blocks ───────────────────
asciidoc_inj = nvim_cfg / "queries" / "asciidoc" / "injections.scm"
marker       = 'injection.language "azadi"'
azadi_snippet = (script_dir.parent / "helix" / "asciidoc-injections.scm").read_text()
if asciidoc_inj.exists() and marker in asciidoc_inj.read_text():
    print("azadi→asciidoc injection already present — skipping")
else:
...
print("  :TSInstall azadi asciidoc asciidoc_inline")
