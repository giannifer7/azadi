...
#
# Stages:
#   glibc   — Debian binary + .deb  (cargo-deb)
#   musl    — Alpine static binary
#   windows — MinGW cross-compiled .exe
#   fedora  — Fedora binary + .rpm  (cargo-generate-rpm)
#
# Usage:
...
RUN dnf install -y curl gcc pkg-config git && dnf clean all
ENV RUSTUP_HOME=/usr/local/rustup \
    CARGO_HOME=/usr/local/cargo \
    PATH=/usr/local/cargo/bin:$PATH
RUN curl https://sh.rustup.rs -sSf \
