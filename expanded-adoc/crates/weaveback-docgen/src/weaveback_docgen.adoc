...
| `--md-root <path>`
| Directory containing generated Markdown projection files
  (default: `<root>/expanded-md`)

| `--md-out-dir <path>`
| Output directory for rendered Markdown HTML (default: `<root>/docs/html-md`)

| `--no-md`
| Skip Markdown HTML rendering.

| `--no-adoc`
| Skip AsciiDoc HTML rendering. Useful for CI checks that validate only the
