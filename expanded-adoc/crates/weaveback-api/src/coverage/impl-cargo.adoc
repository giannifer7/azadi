...
Cargo emits machine-readable compiler messages as JSON envelopes. These structs
model only the fields needed for attribution.
...
Attribution collection traces primary diagnostic spans, or all spans when
building the richer per-span summary.
...
The summary helpers group traced generated spans by source file and source
section so cargo output can point back to literate explanations, not just
generated-code coordinates.
...
The emission helpers preserve cargo's JSON stream and add Weaveback attribution
fields to compiler messages plus a synthetic end-of-stream summary.
