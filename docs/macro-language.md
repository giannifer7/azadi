...
**Variable lookup** reads only the current top frame. Outer `%set` bindings do
not leak into macro bodies; pass values as parameters or freeze them explicitly
with `%alias(..., name=value)`.

**Macro lookup** walks from the top of the stack to frame 0 and returns the
first match. A macro binding in an inner scope shadows the same name in an
outer scope.
...
Looks up `name` in the current scope frame only.
...
* For variables, the evaluated string value is copied.
* For macros, the binding is copied **as-is** — no automatic free-variable
  freezing occurs.
