...
* `weaveback-tangle` block parsing has been split at Rust-file level.
...
* `weaveback-api` cargo coverage attribution has been split at Rust-file level.
  `crates/weaveback-api/src/coverage/cargo.rs` is now a small facade over cargo
  message types, diagnostic attribution collection, source-summary grouping, and
  JSON message emission under `crates/weaveback-api/src/coverage/cargo/`.
* `weaveback-tangle` DB FTS has been split at Rust-file level.
  `crates/weaveback-tangle/src/db/fts.rs` is now a small facade over exported
  result types, helper functions, prose FTS, tags, and embeddings under
  `crates/weaveback-tangle/src/db/fts/`.
* `weaveback-tangle` safe writer has been split at Rust-file level.
  `crates/weaveback-tangle/src/safe_writer.rs` now owns the public error,
  config, and writer type; helper methods, write flow, accessors, and path
  validation live under `crates/weaveback-tangle/src/safe_writer/`.
* `weaveback-macro` CLI integration tests have been split at Rust-file level.
  `crates/weaveback-macro/tests/test_macro_cli.rs` is now a small integration
  test module map, with support code and focused test concerns under
  `crates/weaveback-macro/tests/test_macro_cli/`.
* `weaveback-api` apply-back heuristics have been split at Rust-file level.
...
`crates/weaveback-api/src/coverage/cargo.rs` | 29 | Completed; facade now includes focused `src/coverage/cargo/*.rs` files.
`crates/weaveback-tangle/src/db/fts.rs` | 14 | Completed; facade now includes focused `src/db/fts/*.rs` files.
...
`crates/weaveback-api/src/coverage/lcov/summary.rs` | 300-ish | Further improved; summary types and unattributed-line accounting now live under `src/coverage/lcov/summary/`.
`crates/weaveback-api/src/coverage/tests_coverage/cargo.rs` | 12 | Completed; facade now includes focused `src/coverage/tests_coverage/cargo/*.rs` files.
`crates/weaveback-api/src/coverage/tests_coverage/cargo_extra.rs` | 8 | Completed; facade now includes focused `src/coverage/tests_coverage/cargo_extra/*.rs` files.
